...
- [ ] **CRM activities calendar (Track C):** **`GET /api/crm/activities/calendar?from=<ISO>&to=<ISO>`** (auth) → 200; rows respect **`crmLeadFilter`** (compare with **`GET /api/crm/pipeline`** visibility). UI: **CRM** → calendar toolbar → **`/module/crm/activities/calendar`**; change month → refetch; click an event → lead form. Events are open activities with **`dueAt`** in the visible range.
...
- [ ] **CRM forecast (Track C):** **`GET /api/crm/forecast`** (auth) → 200 with **`weightedPipeline`** and **`stages`**; optional **`?user_id=`** for managers — totals should align with **`GET /api/crm/analytics?user_id=`**. UI: **Opportunity funnel** section when stages have opportunities.
