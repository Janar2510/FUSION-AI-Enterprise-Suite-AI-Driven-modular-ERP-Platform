...
- [ ] **`CRON` workflows** — API bootstrap registers **`startWorkflowCronSchedules`** (with **`campaignWorkflowRunner`** / **`outboxRelay`**). **`Workflow.condition`** must be a valid **`node-cron`** expression (optionally JSON `{ "cron": "..." }`). Schedules reload from DB on a timer (default **120s**; set **`WORKFLOW_CRON_REFRESH_MS`** env to override, minimum **5000**).
