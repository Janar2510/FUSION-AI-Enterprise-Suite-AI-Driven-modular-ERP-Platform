...
- **Prisma workflow rules**: active **`Workflow`** rows with **`trigger`** **`ON_CREATE`** / **`ON_UPDATE`** run after matching model writes (**`AutomationService`**). **`EMAIL`** actions enqueue **`email.send`** via the transactional outbox (relay delivers **`sendEmail`**). **`NOTIFICATION`** writes **`TimelineEvent`** (default **`workflow.notification`**). **`UPDATE_RECORD`** updates a field on **`workflow.model`** inside **`runAutomationSkipped`** (blocklist excludes **`Workflow`**, spine auth, **`OutboxEvent`**, **`AuditLog`**, **`TimelineEvent`**, **`Organization`**, etc.). **`CRON`** uses **`workflowCronBootstrap`**: **`Workflow.condition`** is the schedule; **`syncWorkflowCronSchedules`** reapplies tasks on **`WORKFLOW_CRON_REFRESH_MS`** (default **120s**). **`AutomationService.runCronWorkflow`** runs the action bundle (optional **`action.config.condition`** formula gate).
