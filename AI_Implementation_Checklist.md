...
- **`/api/campaigns`** and **`/api/marketing-web`**: require **`marketing.read`** (GET/list/analytics) and **`marketing.write`** (create/update/delete/launch/compose, website pages/menus/mass mailings/social). Same seed step as automation when keys change.
- **Legacy flat settings**: **`POST /api/settings`** and **`GET /api/settings/users`** require **`settings.write`**. Module namespace stays **`GET /api/settings/:module`** (auth only) and **`PUT`** with **`settings.write`**.
- **`/api/ai`**: **`ai.run`** for **`POST /run`**, list/get/pending; **`ai.approve`** for approve/reject/apply/rollback. Seed adds **`ai.approve`** onto roles that had **`ai.run`** (manager roles).
- **`/api/automation`**: requests require spine permissions **`automation.read`** (list workflows) and **`automation.write`** (create/update/delete/toggle, **`POST /api/automation/cron/sync`** to rescan CRON schedules from DB). After deploy when permission keys change, run **`cd api && npx tsx scripts/seed-roles.ts`**.
- **Prisma workflow rules**: active **`Workflow`** rows with **`trigger`** **`ON_CREATE`** / **`ON_UPDATE`** run after matching model writes (**`AutomationService`**). **`EMAIL`** actions enqueue **`email.send`** via the transactional outbox (relay delivers **`sendEmail`**). **`NOTIFICATION`** writes **`TimelineEvent`** (default **`workflow.notification`**). **`UPDATE_RECORD`** updates **`config.field`** or **`config.fields`** on **`workflow.model`** inside **`runAutomationSkipped`** (blocklist excludes **`Workflow`**, spine auth, **`OutboxEvent`**, **`AuditLog`**, **`TimelineEvent`**, **`Organization`**, etc.). **`SEQUENCE`** chains child actions in **`actions`** (bounded depth/step count). **`WEBHOOK`** **`POST`**s (default) JSON to **`config.url`**; payload **`record`** omits **`__previous`**; **`https`** enforced when **`NODE_ENV=production`**; optional **`config.hmacSecret`** or **`config.hmacSecretEnv`** adds **`X-Fusion-Webhook-Signature`** (**`sha256=`** HMAC-SHA256 of UTF-8 body); **`config.maxRetries`** (0–5, default **2**) retries **5xx** and network failures only. Optional high-volume path: set **`AUTOMATION_WEBHOOK_QUEUE=true`** so eligible **`WEBHOOK`** actions insert **`automation_webhook_deliveries`** (Postgres) and a background worker drains them; failures after **`WEBHOOK_QUEUE_MAX_RUNS`** land in **`status=dead`** (DLQ). Workflows with inline **`hmacSecret`** (not env) stay **synchronous** (secret is not persisted). **`Workflow.condition`** may be legacy snippet or **JSON** rules via **`FormulaService.evaluateWorkflowCondition`**; **`ON_UPDATE`** context includes **`__previous`**. **`CRON`** uses **`workflowCronBootstrap`**: **`Workflow.condition`** is the schedule; **`syncWorkflowCronSchedules`** reapplies tasks on **`WORKFLOW_CRON_REFRESH_MS`** (default **120s**). **`AutomationService.runCronWorkflow`** reads supplementary **`condition`** from the root parsed action’s **`config`** (when the stored JSON is **`SEQUENCE`**, attach **`condition`** to that root **`config`**), then runs **`executeAction`** (cron-only JSON supplementary condition does not veto the tick).
