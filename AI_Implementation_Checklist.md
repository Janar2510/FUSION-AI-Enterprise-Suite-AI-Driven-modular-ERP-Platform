...
- **Prisma workflow rules**: active **`Workflow`** rows with **`trigger`** **`ON_CREATE`** / **`ON_UPDATE`** run after matching model writes (**`AutomationService`**). **`EMAIL`** actions enqueue **`email.send`** via the transactional outbox (relay delivers **`sendEmail`**). **`NOTIFICATION`** writes **`TimelineEvent`** (default **`workflow.notification`**). **`UPDATE_RECORD`** updates a field on **`workflow.model`** inside **`runAutomationSkipped`** to avoid Prisma middleware recursion. **`CRON`** workflows register at API start from **`Workflow.condition`** (cron string or JSON); **`AutomationService.runCronWorkflow`** executes the action bundle (optional **`action.config.condition`** formula gate).
