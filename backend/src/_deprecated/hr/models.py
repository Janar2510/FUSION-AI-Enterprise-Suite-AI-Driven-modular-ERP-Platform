...
class User(Base):
    """User model (referenced by hr models)"""
    __tablename__ = "hr_users_mock"
    id = Column(Integer, primary_key=True, index=True)

class Company(Base):
    """Company model (referenced by hr models)"""
    __tablename__ = "hr_companies_mock"
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)

class Contact(Base):
    """Contact model (referenced by hr models)"""
    __tablename__ = "hr_contacts_mock"
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
...
class HrEmployee(Base):
    ...
    # Core Relations (Odoo Parity)
    user_id = Column(Integer, ForeignKey("hr_users_mock.id"), nullable=True)
    company_id = Column(PG_UUID(as_uuid=True), ForeignKey("hr_companies_mock.id"), nullable=True)
    work_contact_id = Column(PG_UUID(as_uuid=True), ForeignKey("hr_contacts_mock.id"), nullable=True)
    ...
    coach_id = Column(Integer, ForeignKey("hr_employees.id"), nullable=True)
    ...
    work_contact = relationship("Contact")
    job = relationship("HrJob", back_populates="employees")
    department_rel = relationship("HrDepartment", back_populates="employees")
    manager = relationship("HrEmployee", back_populates="subordinates", remote_side=[id])
    coach = relationship("HrEmployee", remote_side=[id], foreign_keys=[coach_id])
    subordinates = relationship("HrEmployee", back_populates="manager", foreign_keys=[manager_id])
...
class HrDepartment(Base):
    ...
    # Relationships
    company = relationship("Company")
    parent = relationship("HrDepartment", back_populates="children", remote_side=[id])
    children = relationship("HrDepartment", back_populates="parent")
    manager = relationship("HrEmployee", foreign_keys=[manager_id])
    employees = relationship("HrEmployee", back_populates="department_rel", foreign_keys="HrEmployee.department_id")
    jobs = relationship("HrJob", back_populates="department")
...
class HrJob(Base):
    ...
    # Relationships
    company = relationship("Company")
    department = relationship("HrDepartment", back_populates="jobs")
    employees = relationship("HrEmployee", back_populates="job")
...
class PerformanceReview(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("hr_employees.id"))
    reviewer_id = Column(Integer, ForeignKey("hr_employees.id"))
    ...
    # Relationships
    employee = relationship("HrEmployee", back_populates="performance_reviews", foreign_keys=[employee_id])
    reviewer = relationship("HrEmployee", foreign_keys=[reviewer_id])
...
class LeaveRequest(Base):
    ...
    # Relationships
    employee = relationship("HrEmployee", back_populates="leave_requests", foreign_keys=[employee_id])
    approver = relationship("HrEmployee", foreign_keys=[approved_by])
...
class TimeEntry(Base):
    ...
    # Relationships
    employee = relationship("HrEmployee", back_populates="time_entries", foreign_keys=[employee_id])
    approver = relationship("HrEmployee", foreign_keys=[approved_by])
