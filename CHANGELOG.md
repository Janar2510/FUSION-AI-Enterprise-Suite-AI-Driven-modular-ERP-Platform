...
- **`/api/ai`** (router `ai-actions.ts`) — **`POST /run`**, **`GET /actions`**, **`GET /actions/:id`**, **`GET /pending`** require **`ai.run`**. **`POST .../approve`**, **`reject`**, **`apply`**, **`rollback`** require **`ai.approve`**. **`DEFAULT_ROLE_PERMISSIONS`** adds **`ai.approve`** alongside **`ai.run`** for every manager role that already had **`ai.run`**. Re-run **`cd api && npx tsx scripts/seed-roles.ts`** after deploy.
