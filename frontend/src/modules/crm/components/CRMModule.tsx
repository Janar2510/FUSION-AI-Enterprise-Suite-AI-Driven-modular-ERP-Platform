...
import { Mail, Phone, DollarSign, Target, Briefcase, ShoppingCart, Plus, CheckCircle2 } from 'lucide-react';
...
const ACTIVITY_TYPES = ['call', 'email', 'meeting', 'task', 'deadline'];

const CrmActivitiesPanel: React.FC<{ leadId: number }> = ({ leadId }) => {
    ...
    const load = async () => {
        try {
            const res = await axios.get(`${API_BASE}/api/crm/leads/${leadId}/activities`);
    ...
    const handleCreate = async () => {
        ...
        setLoading(true);
        try {
            await axios.post(`${API_BASE}/api/crm/leads/${leadId}/activities`, form);
    ...
    const handleDone = async (id: string) => {
        try {
            await axios.patch(`${API_BASE}/api/crm/activities/${id}/done`);
            await load();
        } catch { }
    };

    const TYPE_ICONS: Record<string, string> = {
        call: '📞', email: '✉️', meeting: '📅', task: '✅', deadline: '⏰',
    };
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="space-y-2">
                ...
                {activities.map(a => (
                    <div key={a.id} className={`flex items-start gap-2 p-2 rounded-lg border ${a.doneAt ? 'border-white/5 opacity-50' : 'border-white/10 bg-white/3'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {a.dueAt && (
                                <div className="text-white/40 text-xs">{new Date(a.dueAt).toLocaleDateString()}</div>
                            )}
                        </div>
                        {!a.doneAt && (
                            <button onClick={() => handleDone(a.id)} title="Mark done"
